...
fn synth_gradients(rg: &mut rg::RenderGraph, desc: rg::TextureDesc) -> rg::TextureHandle {
    ...
    pass.render(move |cb, registry| {
        let shader = registry.shader("gradients.spv", RenderShaderType::Compute);
        cb.rg_dispatch_2d(
            &shader,
            output_ref.dims(),
...
fn blur(rg: &mut rg::RenderGraph, input: &rg::TextureHandle) -> rg::TextureHandle {
    ...
    pass.render(move |cb, registry| {
        let shader = registry.shader("blur.spv", RenderShaderType::Compute);
        cb.rg_dispatch_2d(
            &shader,
            input_ref.dims(),
            &[RenderShaderArgument {
...
fn into_ycbcr(rg: &mut rg::RenderGraph, mut input: rg::TextureHandle) -> rg::TextureHandle {
    ...
    pass.render(move |cb, registry| {
        let shader = registry.shader("into_ycbcr.spv", RenderShaderType::Compute);
        cb.rg_dispatch_2d(
            &shader,
            input_ref.dims(),
            &[RenderShaderArgument {
