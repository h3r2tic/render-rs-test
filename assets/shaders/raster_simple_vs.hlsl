...
float3 unpack_unit_direction_11_10_11(uint pck) {
    return float3(
        float(pck & ((1u << 11u) - 1u)) * (2.0f / float((1u << 11u) - 1u)) -
            1.0f,
        float((pck >> 11u) & ((1u << 10u) - 1u)) *
                (2.0f / float((1u << 10u) - 1u)) -
            1.0f,
...
VsOut main(uint vid: SV_VertexID) {
    ...
    vsout.position =
        mul(constants.camera.view_to_clip,
            mul(constants.camera.world_to_view, float4(v.position, 1)));
