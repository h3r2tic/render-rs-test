use crate::{GpuRt, Ref, Texture};
use render_core::{
    constants::MAX_RENDER_TARGET_COUNT,
    state::{RenderDrawState, RenderScissorRect, RenderViewportRect},
    types::RenderTargetInfo,
};

pub struct ColorAttachment {
    pub texture: Ref<Texture, GpuRt>,
    pub info: RenderTargetInfo,
}

impl From<(Ref<Texture, GpuRt>, RenderTargetInfo)> for ColorAttachment {
    fn from(v: (Ref<Texture, GpuRt>, RenderTargetInfo)) -> Self {
        Self {
            texture: v.0,
            info: v.1,
        }
...
macro_rules! impl_into_color_attachments {
    ($count:expr, $($elems:ident),*) => {
        impl IntoColorAttachments for [(Ref<Texture, GpuRt>, RenderTargetInfo); $count] {
